...
factory = {
	...
	locations: [
		...
		{
			name: 'jps_upload',
			regex: /http(s)?:\/\/(.*\.)?jpopsuki\.eu\/upload\.php.*/i,
		},
		{
	...
	insertIntoPage(element) {
		switch (this.currentLocation) {
			...
			case 'pth_edit': {
				...
				summaryInput.parentNode.insertAfter(
					element,
			...
			case 'jps_upload': {
				...
				const f = function (rawData) {
					...
					if (currentTarget === 'other') {
						...
						albumTitleInput = document.querySelector('#title');
						yearInput = document.querySelector('#releasedate');
					}

					if (/itunes/.test(rawData.url)) {
						const releaseTypeInput = document.querySelector('#categories');
						switch (true) {
							case /.+ - Single$/.test(rawData.title):
								rawData.title = rawData.title.replace(/ - Single$/, '');
								if (releaseTypeInput.getAttribute('disabled') !== 'disabled') {

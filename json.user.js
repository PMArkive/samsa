...
(function() {
	...
	ChooseJSONParseBtn.addEventListener('click', function (evt) {
		...
		if (file) {
			sourceWebsiteIndex = /NWCD .+\.json/.test(file.name) ? 0 : (/.+ \[orpheus\.network\]\.json/.test(file.name) ? 1 : -1);
				...
				reader.onload = function (evt) {
					...
					switch(categoryIndex) {
						case 0:
							...
							var albumArtist = document.getElementById('artist');
							var albumArtistJSON = releaseJSON.response.group.musicInfo.artists[0].name;

							if (albumArtistJSON != '') {
								albumArtist.value = albumArtistJSON;
							} else {
								alert('No artists is included in the JSON!');
							}
							...
							var initialYear = document.getElementById('releasedate');
							...
							var releaseType = document.getElementById('categories');
							var releaseTypes = [];
							...
							if (edition) {
								document.getElementById('remaster').checked = true;
								window.eval('Remaster();');
